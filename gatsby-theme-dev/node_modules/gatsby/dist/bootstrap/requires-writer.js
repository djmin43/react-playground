...
const pickComponentFields = page => (0, _pick2.default)(page, [`component`, `componentChunkName`]);

const getComponents = pages => (0, _orderBy2.default)((0, _uniqBy2.default)((0, _map2.default)(pages, pickComponentFields), c => c.componentChunkName), c => c.componentChunkName);
/**
...
const writeAll = async state => {
  ...
  if (process.env.GATSBY_EXPERIMENTAL_DEV_SSR) {
    ...
    const lazySyncRequires = `exports.ssrComponents = {\n${cleanedSSRVisitedPageComponents.map(c => `  "${c.componentChunkName}": require("${(0, _gatsbyCoreUtils.joinPath)(c.component)}")`).join(`,\n`)}
  }\n\n`;
    (0, _gatsbyWebpackVirtualModules.writeModule)(`$virtual/ssr-sync-requires`, lazySyncRequires); // if this is executed, webpack should mark it as invalid, but sometimes there is some timing race
...
\n\n`;
  ...
  if (process.env.gatsby_executing_command === `develop`) {
    asyncRequires = `exports.components = {\n${components.map(c => {
      ...
      return `  "${c.componentChunkName}": () => import("${(0, _gatsbyCoreUtils.slash)(`./${relativeComponentPath}`)}?export=default" /* webpackChunkName: "${c.componentChunkName}" */)`;
    }).join(`,\n`)}
}\n\n
...
exports.head = {\n${components.map(c => {
      ...
      return `  "${c.componentChunkName}": () => import("${(0, _gatsbyCoreUtils.slash)(`./${relativeComponentPath}`)}?export=head" /* webpackChunkName: "${c.componentChunkName}head" */)`;
    }).join(`,\n`)}
}\n\n`;
  } else {
    asyncRequires = `exports.components = {\n${components.map(c => {
