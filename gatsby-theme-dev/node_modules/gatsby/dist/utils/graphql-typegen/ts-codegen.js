...
exports.DEFAULT_TYPES_OUTPUT_PATH = void 0;
...
const NAMESPACE = `Queries`; // These override the defaults from
...
async function writeTypeScriptTypes(directory, schema, definitions, graphqlTypegenOptions) {
  ...
  let gatsbyNodeDocuments = []; // The loadDocuments + CodeFileLoader looks for graphql(``) functions inside the gatsby-node.ts files
  ...
  try {
    gatsbyNodeDocuments = await (0, _load.loadDocuments)([`./gatsby-node.ts`, `./plugins/**/gatsby-node.ts`], {
  ...
  } catch (e) {// These files might not exist, so just skip this
  }
  ...
  const result = await (0, _core.codegen)({ ...pluginConfig,
    ...codegenOptions
  });
