...
async function startServer(program, app, workerPool = WorkerPool.create()) {
  ...
  (0, _loadingIndicator.writeVirtualLoadingIndicatorModule)(); // Remove the following when merging GATSBY_EXPERIMENTAL_DEV_SSR
  ...
  const createIndexHtml = async activity => {
    try {
      ...
      await doBuildPages(rendererPath, [`/`], activity, workerPool, _types.Stage.DevelopHTML); // close the compiler
  ...
  const graphqlEndpoint = `/_+graphi?ql`; // TODO(v5): Remove GraphQL Playground (GraphiQL will be more future-proof)
  ...
  app.get(`/page-data/:pagePath(*)/page-data.json`, async (req, res, next) => {
    ...
    if (page) {
      try {
        ...
        let pageData; // TODO move to query-engine
  ...
  app.get(`/__original-stack-frame`, (req, res) => {
    var _res$locals, _res$locals$webpack, _res$locals$webpack$d, _res$locals$webpack$d2, _req$query, _ref, _req$query2, _ref2, _req$query3, _compilation$codeGene, _compilation$codeGene2;
    ...
    const moduleId = (_req$query = req.query) === null || _req$query === void 0 ? void 0 : _req$query.moduleId;
    const lineNumber = parseInt((_ref = (_req$query2 = req.query) === null || _req$query2 === void 0 ? void 0 : _req$query2.lineNumber) !== null && _ref !== void 0 ? _ref : 1, 10);
    const columnNumber = parseInt((_ref2 = (_req$query3 = req.query) === null || _req$query3 === void 0 ? void 0 : _req$query3.columnNumber) !== null && _ref2 !== void 0 ? _ref2 : 1, 10);
    let fileModule;
  ...
  app.get(`/__file-code-frame`, async (req, res) => {
    ...
    const filePath = (_req$query4 = req.query) === null || _req$query4 === void 0 ? void 0 : _req$query4.filePath;
    const lineNumber = parseInt((_req$query5 = req.query) === null || _req$query5 === void 0 ? void 0 : _req$query5.lineNumber, 10);
    const columnNumber = parseInt((_req$query6 = req.query) === null || _req$query6 === void 0 ? void 0 : _req$query6.columnNumber, 10);
  ...
  app.use((0, _expressMiddlewares.configureTrailingSlash)(() => _redux.store.getState(), trailingSlash)); // Disable directory indexing i.e. serving index.html from a directory.
  ...
  app.use(async (req, res) => {
    ...
    if (fullUrl.endsWith(`app-data.json`)) {
      ...
      if (process.env.GATSBY_EXPERIMENTAL_DEV_SSR) {
        ...
        const page = (0, _findPageByPath.findPageByPath)(_redux.store.getState(), potentialPagePath, false); // When we can't find a page we send 404
  ...
  if (!process.env.GATSBY_EXPERIMENTAL_DEV_SSR) {
    const chokidar = require(`chokidar`); // Register watcher that rebuilds index.html every time html.js changes.
