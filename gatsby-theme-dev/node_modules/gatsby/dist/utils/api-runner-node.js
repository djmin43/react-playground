...
const nodeMutationsWrappers = {
  getNode(id) {
    return (0, _detectNodeMutations.wrapNode)(getNode(id));
  },

  getNodes() {
    return (0, _detectNodeMutations.wrapNodes)(getNodes());
  },

  getNodesByType(type) {
    return (0, _detectNodeMutations.wrapNodes)(getNodesByType(type));
  },

  getNodeAndSavePathDependency(id) {
    return (0, _detectNodeMutations.wrapNode)(getNodeAndSavePathDependency(id));
  }
...
const runAPI = async (plugin, api, args, activity) => {
  ...
  if (gatsbyNode[api]) {
    ...
    const tracing = initAPICallTracing(pluginSpan); // See https://github.com/gatsbyjs/gatsby/issues/11369

    const cache = api === `onPreInit` ? getUninitializedCache(plugin.name) : getCache(plugin.name); // Ideally this would be more abstracted and applied to more situations, but right now
    ...
    const apiCallArgs = [{ ...args,
      ...
      emitter,
      getCache,
      getNodes: shouldDetectNodeMutations ? nodeMutationsWrappers.getNodes : getNodes,
      getNode: shouldDetectNodeMutations ? nodeMutationsWrappers.getNode : getNode,
      getNodesByType: shouldDetectNodeMutations ? nodeMutationsWrappers.getNodesByType : getNodesByType,
      reporter: extendedLocalReporter,
      getNodeAndSavePathDependency: shouldDetectNodeMutations ? nodeMutationsWrappers.getNodeAndSavePathDependency : getNodeAndSavePathDependency,
      cache,
...
function apiRunnerNode(api, args = {}, {
  ...
  if (api === `sourceNodes` && args.pluginName) {
    implementingPlugins = implementingPlugins.filter(plugin => plugin.name === args.pluginName);
  ...
  return new Promise(resolve => {
    ...
    runPromise(implementingPlugins, plugin => {
      ...
      if (api === `onCreateNode` && gatsbyNode !== null && gatsbyNode !== void 0 && gatsbyNode.unstable_shouldOnCreateNode && // Don't bail if this api is not exported
