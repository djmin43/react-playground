...
async function findPageOwnedByNode({
  ...
  let pagePath = firstPagePathWithNodeAsDataDependency || firstPagePathWithNodeInGraphQLListField; // for static queries, we can only find the first page using that static query
...
async function processNodeManifest(inputManifest, listOfUniqueErrorIds, nodeManifestPagePathMap, verboseLogs, previouslyWrittenNodeManifests) {
  ...
  const previouslyWrittenNodeManifest = await previouslyWrittenNodeManifests.get(inputManifest.manifestId); // write a manifest if we don't currently have one written for this ID
  ...
  if (shouldWriteManifest) {
    const writePromise = _fsExtra.default.writeJSON(manifestFilePath, finalManifest); // This prevents two manifests from writing to the same file at the same time
