...
class LocalNodeModel {
  ...
  getAllNodes(args, pageDependencies = {}) {
    // TODO(v5): Remove API
    (0, _reportOnce.reportOnce)(`nodeModel.getAllNodes() is deprecated. Use nodeModel.findAll() with an empty query instead.`);
    const {
      type = `Node`
    } = args || {};
    let result;
    ...
    if (type === `Node`) {
      result = (0, _datastore.getNodes)();
    } else {
      const nodeTypeNames = (0, _utils.toNodeTypeNames)(this.schema, type);
      const nodesByType = nodeTypeNames.map(typeName => (0, _datastore.getNodesByType)(typeName));
    ...
    if (typeof pageDependencies.connectionType === `undefined`) {
      pageDependencies.connectionType = typeof type === `string` ? type : type.name;
    }
  ...
  async runQuery(args, pageDependencies = {}) {
    // TODO(v5): Remove API
    (0, _reportOnce.reportOnce)(`nodeModel.runQuery() is deprecated. Use nodeModel.findAll() or nodeModel.findOne() instead.`);
    ...
    const result = await this.findAll({ ...args,
      query
    }, pageDependencies);
  ...
  async findAll(args, pageDependencies = {}) {
    const {
      gqlType,
      ...result
    } = await this._query(args, pageDependencies); // Tracking connections by default:
...
class ContextualNodeModel {
  ...
  getAllNodes(args, pageDependencies) {
    return this.nodeModel.getAllNodes(args, this._getFullDependencies(pageDependencies));
  }
