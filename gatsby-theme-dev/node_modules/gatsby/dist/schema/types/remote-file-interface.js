...
function addRemoteFileInterfaceFields(schemaComposer, typeComposer) {
  const remoteFileInterfaceType = getOrCreateRemoteFileInterface(schemaComposer); // eslint-disable-next-line @typescript-eslint/no-explicit-any

  typeComposer.addFields(remoteFileInterfaceType.getFields());
}

function getOrCreateRemoteFileInterface( // eslint-disable-next-line @typescript-eslint/no-explicit-any
schemaComposer) {
  ...
  return schemaComposer.getOrCreateIFTC(`RemoteFile`, tc => {
    ...
    const boundActions = (0, _redux.bindActionCreators)(_index2.actions, _index.store.dispatch); // @ts-ignore - types are messed up by schema composer maybe new version helps here
