...
const isTestEnv = process.env.NODE_ENV === `test`; // Memoize function used to pick shadowed page components to avoid expensive I/O.
...
${reservedFields.map(f => `  * "${f}"`).join(`\n`)}
  ...
  const {
    ...
    message,
    panicOnBuild
  } = validatePageComponent(page, rootPath, name);
  ...
  if (isNotTestEnv) {
    if (pageComponentCache.has(page.component)) {
      ...
      try {
        ...
        const commonDir = getCommonDir(rootPath, page.component); // using `path.win32` to force case insensitive relative path
...
actions.deleteNode = (node, plugin) => {
  const id = node && node.id; // Always get node from the store, as the node we get as an arg
  ...
  if (plugin) {
    ...
    if (internalNode && typeOwners[internalNode.internal.type] && typeOwners[internalNode.internal.type] !== pluginName) throw new Error(stripIndent`
          The plugin "${pluginName}" deleted a node of a type owned by another plugin.

          The node type "${internalNode.internal.type}" is owned by "${typeOwners[internalNode.internal.type]}".
  ...
  const deleteAction = createDeleteAction(internalNode); // It's possible the file node was never created as sometimes tools will
