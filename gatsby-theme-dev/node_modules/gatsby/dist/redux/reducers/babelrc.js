    ...
    case `SET_BABEL_PLUGIN`:
      {
        Object.keys(state.stages).forEach(stage => {
          ...
          const index = (0, _findIndex2.default)(state.stages[stage].plugins, plugin => plugin.name === action.payload.name); // If the plugin already exists, merge the options.
    ...
    case `SET_BABEL_PRESET`:
      {
        Object.keys(state.stages).forEach(stage => {
          ...
          const index = (0, _findIndex2.default)(state.stages[stage].presets, plugin => plugin.name === action.payload.name); // If the plugin already exists, merge the options.
          // Otherwise, add it to the end.
