...
const createGlobArray = (siteDirectoryPath, plugins) => {
  ...
  function globIgnorePatterns(root, pluginName) {
    ...
    return [`${root}/src/api${nestedFolder}__tests__/**/*.+(js|ts)`, // Jest tests
...
const createWebpackConfig = async ({
  ...
  _fsExtra.default.writeFileSync(_path2.default.join(compiledFunctionsDir, `manifest.json`), JSON.stringify(knownFunctions, null, 4)); // Load environment variables from process.env.* and .env.* files.
  ...
  return {
    ...
    module: {
      rules: [// Webpack expects extensions when importing ESM modules as that's what the spec describes.
  ...
  try {
    ...
    await new Promise(async (resolve, reject) => {
      ...
      function callback(err, stats) {
        ...
        const errors = (stats === null || stats === void 0 ? void 0 : stats.compilation.errors) || []; // If there's errors, reject in production and print to the console
        // in development.
