  ...
  if (transpiled && transpiled.length) {
    for (const item of transpiled) {
      try {
        ...
        ast = tmp;
        break;
      } catch (error) {// We emit the actual error below if every transpiled variant fails to parse
      }
    }
...
class FileParser {
  ...
  async parseFile(file, addError) {
    ...
    if (!text.includes(`graphql`) && !text.includes(`gatsby-plugin-image`) && !text.includes(`getServerData`) && !text.includes(`config`)) {
      return null;
    }
