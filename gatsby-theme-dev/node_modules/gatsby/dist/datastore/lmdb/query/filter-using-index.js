...
function getMostSpecificQuery(queries, [indexField]) {
  const fieldQueries = getFieldQueries(queries, indexField); // Assuming queries are sorted by specificity, the best bet is to pick the first query
    ...
    case _query.DbComparator.LTE:
      {
        ...
        const start = (_resolveRangeEdge = resolveRangeEdge(context, field, _query.DbComparator.GTE)) !== null && _resolveRangeEdge !== void 0 ? _resolveRangeEdge : resolveRangeEdge(context, field, _query.DbComparator.GT, ValueEdges.AFTER); // Do not include null or undefined in results unless null was requested explicitly
        //
        // Index ordering:
