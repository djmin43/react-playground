...
const getGatsbyNodePartial = (node, indexFields, resolvedFields) => {
  ...
  if (nodeIdToIdentifierMap.has(cacheKey)) {
    ...
    derefPartial = (_nodeIdToIdentifierMa = nodeIdToIdentifierMap.get(cacheKey)) === null || _nodeIdToIdentifierMa === void 0 ? void 0 : _nodeIdToIdentifierMa.deref(); // now check if we have it in memory and it has all the fields we need
...
const getNodesFromCacheByValue = (filterCacheKey, filterValue, filtersCache, wasElemMatch) => {
  ...
  if (op === `$in`) {
    ...
    const set = new Set(); // TODO: we can also mergeSort for every step. this may perform worse because of how memory in js works.
    ...
    arr.sort(sortByIds); // Note: it's very unlikely that the list of filter values is big so .includes should be fine here
  ...
  if (op === `$regex`) {
    ...
    arr.sort(sortByIds); // elemMatch can cause a node to appear in multiple buckets so we must dedupe
