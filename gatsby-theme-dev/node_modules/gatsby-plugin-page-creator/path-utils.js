...
const extractModelRegex = /\{([a-zA-Z_][\w]*)\./; // Given a absolutePath that has a collection marker it will extract the Model.
// /foo/bar/{Model.bar} => Model

function extractModel(absolutePath) {
  const model = extractModelRegex.exec(absolutePath); // This shouldn't happen - but TS requires us to validate
...
const curlyBracesContentsRegex = /\{.*?\}/g; // This extracts all information in an absolute path to an array of each collection part
// /foo/{Model.bar}/{Model.baz} => ['Model.bar', 'Model.baz']

function extractAllCollectionSegments(absolutePath) {
  const slugParts = absolutePath.match(curlyBracesContentsRegex); // This shouldn't happen - but TS requires us to validate
...
const extractFieldRegexCurlyBraces = /[{}]/g; // Regex created with: https://spec.graphql.org/draft/#sec-Names
// First char only letter, underscore; rest letter, underscore, digit
...
const switchToPeriodDelimitersRegex = /__/g; // Used to convert filePath field accessors to js syntax accessors.
// e.g.
//   foo__bar => foo.bar
...
const convertUnionSyntaxToGraphqlRegex = /\(/g; // Converts the part of the file from something like `(Union)` to `... on Union`
