...
function generateQueryFromString(queryOrModel, fileAbsolutePath) {
  ...
  const fields = extractUrlParamsForQuery(fileAbsolutePath); // In case queryOrModel is not capitalized
...
function reverseLookupParams(queryResults, absolutePath) {
  ...
  absolutePath.split(_path.default.sep).forEach(part => {
    ...
    extracted.forEach(extract => {
      if (extract === ``) return;
      const results = (0, _get2.default)(queryResults.nodes ? queryResults.nodes[0] : queryResults, // replace __ with accessors '.'
...
function extractUrlParamsForQuery(createdPath) {
  const parts = createdPath.split(_path.default.sep); // always add `id` to queries
