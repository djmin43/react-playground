...
function getCollectionRouteParams(urlTemplate, urlPath) {
  const params = {}; // Remove file extension first so that urlTemplate and urlPath have the same shape

  const cleanedUrlTemplate = (0, _pathUtils.removeFileExtension)(urlTemplate);
  const urlTemplateParts = cleanedUrlTemplate.split(`/`); // Create a regex string for later use by creating groups for all { } finds
