...
function preset(_, options = {}) {
  ...
  if (reactImportSource && options.reactRuntime !== `automatic`) {
    throw Error(`\`@babel/preset-react\` requires reactRuntime \`automatic\` in order to use \`importSource\`.`);
  }
  ...
  return {
    presets: [[resolve(`@babel/preset-env`), {
      ...
      ...(reactImportSource && {
        importSource: reactImportSource
      })
    }]],
    plugins: [[resolve(`./optimize-hook-destructuring`), {
      ...
      loose: true
    }], [resolve(`@babel/plugin-proposal-nullish-coalescing-operator`)], [resolve(`@babel/plugin-proposal-optional-chaining`)], resolve(`babel-plugin-macros`), resolve(`@babel/plugin-syntax-dynamic-import`), [resolve(`@babel/plugin-transform-runtime`), {
      corejs: false,
      helpers: true,
      regenerator: true,
    ...
    isBrowser && [resolve(`@babel/plugin-transform-spread`), {
      loose: false // Fixes #14848

    }], isBrowser && [resolve(`@babel/plugin-transform-classes`), {
      loose: true
    }], IS_TEST && resolve(`babel-plugin-dynamic-import-node`), stage === `build-javascript` && [// Remove PropTypes from production build
