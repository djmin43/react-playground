...
async function fetchFile({
  ...
  await fetchFileMutex.acquire(); // Fetch the file.
  ...
  try {
    ...
    let filename = createFilePath(finalDirectory, name, ext); // See if there's response headers for this url
...
function getInFlightObject(key, buildId) {
  ...
  const remoteFile = getStorage(getDatabaseDir()).remoteFileInfo.get(key); // if buildId match we know it's the same build and it already processed this url this build
...
async function setInFlightObject(key, buildId, value) {
  ...
  await getStorage(getDatabaseDir()).remoteFileInfo.put(key, { ...value,
    buildId
  });
}